<template>
  <div class="tf-container container">
        ...
        <row :gutter="20">
          ...
          <i-col span="12">
            <formItem class="btn-box">
              ...
              <i-button type="warning" class="btn btn-add" @click="checkAddMode">添加</i-button>
              <i-button type="success" class="btn" @click="xiugai" v-if="btnIsshow" :disabled="!currentID">修改</i-button>
              <i-button type="warning" class="btn btn-purple" @click="checkBillStatus" :disabled="!currentID">浏览</i-button>
            </formItem>
          </i-col>
        </row>
      </Form>
    ...
    <!-- 弹窗 -->
    <Modal :mask-closable="false" v-model="modal1" class="dialog" width="1200">
      ...
      <Form ref="formValidate1" :model="formValidate" :label-width="120" @submit.native.prevent class="form">
        <row type="flex" justify="space-between">
          <i-col span="6">
            <FormItem prop="CustomerPhone" label="电话：">
              <Input v-model="formValidate.CustomerPhone" placeholder="电话" :readonly="true"></Input>
            </FormItem>
          </i-col>
          <i-col span="6">
            <FormItem prop="CustomerName" label="姓名：">
              <Input v-model="formValidate.CustomerName" placeholder="姓名" :readonly="true"></Input>
            </FormItem>
          </i-col>
          <i-col span="6">
            <FormItem label="年龄：" prop="CustomerAge">
              <Input v-model="formValidate.CustomerAge" placeholder="年龄" :readonly="true"></Input>
            </FormItem>
          </i-col>
          <i-col span="6">
            <FormItem prop="CustomerSex" label="性别：">
              <Input v-model="formValidate.CustomerSex" placeholder="性别" :readonly="true"></Input>
            </FormItem>
          </i-col>
        </row>
        <row type="flex" justify="space-between">
        ...
        <row type="flex">
          ...
          <i-col span="6" v-if="mode==2">
            <FormItem label="还款日期：" prop="dataTime">
              <DatePicker class="DatePicker_time" type="date" :options="optionsDate" placeholder="开始日期" v-model="dataTime" :clearable="false" :editable="false"></DatePicker>
            </FormItem>
          </i-col>
          <i-col span="6" v-if="mode==2">
            <FormItem label="修改时间：">
      ...
      <!-- 支付方式 浏览用 -->
      <div class="table-box" v-if="mode==3">
        <i-table stripe :columns="chargeTablePrintHead" :data="chargeTableData"></i-table>
      </div>

      <Input v-model="formValidate.Remark" placeholder="无" type="textarea" :rows="4" style="margin-top:20px;" v-if="mode == 2"></Input>
      ...
      <div slot="footer" class="footer not-print" v-if="mode == 1">
        <Button class="btn-sure" type="primary" @click="addForm" :loading="uploading" :disabled="jinyong">保存添加</Button>
      ...
      <div slot="footer" class="footer not-print" v-if="mode ==2">
        <Button class="btn-sure" type="primary" @click="EditForm" :loading="uploading" :disabled="jinyong">保存修改</Button>
        <Button class="btn-cancel" type="warning" @click="resetForm" :loading="uploading">取消</Button>
      </div>
    </Modal>
    <!-- 打印格式 -->
...
export default {
  ...
  watch: {
     $route(to, from) {  
        //监听路由是否变化  
      if (this.$route.query.CustPhone) {
        // 判断条件1  判断传递值的变化 console.log(this.$route.query.CustomerID); 
        let regex = /^1\d{10}$/;
        if (!regex.test(this.$route.query.CustPhone)) {
          this.$Message.error("电话号码格式有误");
          return false;
        }
        this.ArrBackLink(this.$route.query.CustPhone); 
        this.$router.push({
  ...
  methods: {
          ...
          api.getCustomerByPhone(data).then(res => {
           ...
           let a = {
            ...
            if (res.error_code === "Success") {
              ...
              that.setData(a);
              that.addCharge();
              that.getArrearsListByCustomerID(res.data.ID);
            } else {
    ...
    checkBillStatusX() {
      ...
      api.seeRefundsPrepayments(that.currentID).then(response => {
        ...
        that.chargeTableData = res.PayDetails;
        that.setData(res);
        let b = {
    ...
    BtnChangeDate() {
      ...
      let data = {
        ID: that.currentID,
        Date: moment(that.dataTime).format("YYYY-MM-DD HH:mm:ss")
      };
      api.arrearsRecoveryEditDate(data).then(response => {
        if (response.error_code === "Success") {
          that.modal1 = false;
    ...
    EditForm() {
      ...
      let data = {
        ...
        HandleBy: that.userMes.EmployeeID,
        Remark: that.formValidate.Remark,
        Date: echartsCommon.changeTimeHms(new Date()),
        Details: []
      };
      // 支付明细
      data.CertainProjectID.push(that.CertainProjectID);
      let chargeDatas = that.chargeTableData;
      for (let i in chargeDatas) {
        let a = {
          PayModeID: chargeDatas[i].PayModeID,
          HandlingFee: chargeDatas[i].HandlingFee,
          Amount: chargeDatas[i].Amount
        };
        data.Details.push(a);
      }
      for (let i of that.chargeTableData) {
      ...
      // 总额
      that.Amount = data.RepaymentAmount;
      api.arrearsRecoveryEdit(data).then(response => {
        that.uploading = false;
        if (response.error_code === "Success") {
          that.resetSearch();
          that.printPage();
  ...
  mounted() { 
    let that = this;
    that.btnIsshow = btnQX.apiUrlList("arrearsRecovery"); //权限控制修改按钮
    that.$nextTick(() => {
