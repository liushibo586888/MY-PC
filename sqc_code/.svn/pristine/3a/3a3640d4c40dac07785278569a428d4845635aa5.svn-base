  ...
  <div class="QYjysh-container container">
    <div>
      <Tabs value="name1" @on-click="tabChangeQYjysh">
        ...
        <TabPane label="新客渠道分类客单价" icon="dot" name="name17">
          <keep-alive>
        ...
        <row :gutter="20">
          ...
          <i-col span="3" v-if="tabIndex2=='name9'">
            <FormItem prop="CategoryID">
              <Select v-model="formItem.CategoryID" placeholder="物资">
                <Option value="all" @click.native="chooseCategory()">所有物资</Option>
                <Option v-for="(item, index) in CategoryList" :value="item.ID" :key="index" @click.native="chooseCategory(item.Name,item.ID)">{{ item.Name }}</Option>
              </Select>
            </FormItem>
          </i-col>
          <i-col span="3" v-if="tabIndex2=='name14'||tabIndex2=='name16'">
            <FormItem prop="ItemCategoryID">
              <Select v-model="formItem.ItemCategoryID" placeholder="选择项目">
                <Option v-for="(item, index) in zxxmList" :value="item.ID" :key="index" @click.native="choose1(item.Name,item.ID)">{{item.Name}}</Option>
              </Select>
            </FormItem>
          </i-col>
          <i-col span="3" v-if="tabIndex2=='name17'||tabIndex2=='name15'">
            <FormItem prop="UnderstandWayID">
              <Select v-model="formItem.UnderstandWayID" placeholder="渠道">
                <Option value="all" @click.native="chooseQD()">所有渠道</Option>
                <Option v-for="(item, index) in UnderstandWayList" :value="item.ID" :key="index" @click.native="chooseQD(item.Name,item.ID)">{{ item.Name }}</Option>
              </Select>
            </FormItem>
          </i-col>
          <i-col :span='span'>
