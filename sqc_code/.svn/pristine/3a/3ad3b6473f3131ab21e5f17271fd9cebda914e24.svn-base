<template>
  <div class="sf-container container">
    ...
    <div class="table-box" id="tableBox">
      <Table :columns="columns1" :data="list" highlight-row :row-class-name="rowClassName" :height="setTableHeight"
        ref="mainTable" @on-current-change="getRowID"></Table>
        ...
        <row type="flex">
          ...
          <i-col span="6">
            <FormItem label="修改时间：" prop="changeTime">
              <DatePicker class="DatePicker_time" type="date" :options="options1" placeholder="修改时间"
                v-model="formValidate.changeTime" :clearable="false" :editable="false"></DatePicker>
            </FormItem>
          </i-col>
          <i-col span="6" v-if="isShow">
            <FormItem label="保存修改时间：" prop="changeTime">
              <Button class="btn-cancel" type="warning" style="width:100px" @click="BtnaddForm"
                :loading="Btnloading">点击保存</Button>
            </FormItem>
          </i-col>
        </row>
        <!-- ** -->
...
<script>
  ...
  export default {
    ...
    mounted() {
      let that = this;
      that.isShow = btnQX.apiUrlList("xmzfglTime"); //控制修改时间按钮
      that.$nextTick(() => {
