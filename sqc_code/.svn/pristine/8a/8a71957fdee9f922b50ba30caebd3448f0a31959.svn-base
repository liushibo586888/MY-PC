<template>
  <div class="xmqz-container container">
        ...
        <row :gutter="20">
          <i-col span="3">
            <formItem prop="startDate">
              ...
              <DatePicker class="DatePicker_time" type="date" :options="options1" placeholder="开始日期"
                v-model="formItem.startDate" :clearable="false" :editable="false"></DatePicker>
            </formItem>
          </i-col>
          <i-col span="3">
            <formItem prop="endDate">

              <DatePicker class="DatePicker_time" type="date" :options="options1" placeholder="结束日期"
                v-model="formItem.endDate" :clearable="false" :editable="false"></DatePicker>
            </formItem>
          </i-col>
          <i-col span="3">
          ...
          <i-col span="12">
            <formItem class="btn-box">
              ...
              <i-button type="primary" class="btn btn-purple" @click="changeMode(1)" v-if="showEdit"
                :disabled="!currentID">修改</i-button>
              <i-button type="error" class="btn" @click="changeMode(2)" v-if="showLook" :disabled="!currentID">浏览
              </i-button>
            </formItem>
          </i-col>
        </row>
    ...
    <div class="table-box" id="tableBox">
      <Table :columns="columns1" :data="list" highlight-row :height="setTableHeight" ref="mainTable"
        @on-current-change="chooseRow"></Table>
    ...
    <Modal :mask-closable="false" v-model="modal1" width="1000" :scrollable="true" class="dialog"
        ...
        <row type="flex" justify="space-between">
          <i-col span="6">
            <FormItem prop="CustomerName" label="姓名：">
              <Input v-model="formValidate.CustomerName" placeholder="无" :disabled="true" />
            </FormItem>
          </i-col>
          <i-col span="6">
            <FormItem prop="CustomerPhone" label="电话：">
              <Input v-model="formValidate.CustomerPhone" placeholder="无" :disabled="true" />
            </FormItem>
          </i-col>
          <i-col span="6">
            <FormItem label="年龄：" prop="CustomerAge">
              <Input v-model="formValidate.CustomerAge" placeholder="无" :disabled="true" />
            </FormItem>
          </i-col>
          <i-col span="6">
            <FormItem prop="CustomerSex" label="性别：">
              <Input v-model="formValidate.CustomerSex" placeholder="无" :disabled="true" />
            </FormItem>
          </i-col>
        </row>
        <row type="flex" justify="space-between">
          <i-col span="6">
            <FormItem label="预交余额：" prop="CashBalance">
              <InputNumber v-model="formValidate.CashBalance" placeholder="无" :disabled="true" :precision="2"
                style="width:100%;"></InputNumber>
            </FormItem>
          </i-col>
          <i-col span="6">
            <FormItem label="积分余额：" prop="IntegrationBalance">
              <InputNumber v-model="formValidate.IntegrationBalance" placeholder="无" :disabled="true" :precision="2"
                style="width:100%;"></InputNumber>
            </FormItem>
          </i-col>
          <i-col span="6">
            <FormItem label="欠款余额：" prop="ArrearsBalance">
              <InputNumber v-model="formValidate.ArrearsBalance" placeholder="无" :disabled="true" :precision="2"
                style="width:100%;"></InputNumber>
            </FormItem>
          </i-col>
          <i-col span="6">
            <FormItem label="赠送余额：" prop="HandselBalance">
              <InputNumber v-model="formValidate.HandselBalance" placeholder="无" :disabled="true" :precision="2"
                style="width:100%;"></InputNumber>
            </FormItem>
          </i-col>
        </row>
        <row type="flex">
          <i-col span="6">
            <FormItem label="美肤总监：" prop="SceneEmployeeName">
              <Input v-model="formValidate.SceneEmployeeName" placeholder="无" :disabled="true" />
            </FormItem>
          </i-col>
          <i-col span="6">
            <FormItem label="渠道咨询：" prop="TMKEMployeeName">
              <Input v-model="formValidate.TMKEMployeeName" placeholder="无" :disabled="true" />
            </FormItem>
          </i-col>
          ...
          <i-col span="6">
            ...
            <!-- <FormItem label="护理师：" prop="BeauticianID">
                          <Input v-model="formValidate.BeauticianID" placeholder="无" />
                      </FormItem> -->

            <FormItem label="护理师：" prop="BeauticianID" :rules="formValidateRule.BeauticianID">
              <Select v-model="formValidate.BeauticianID" :filterable="true" placeholder="请选择">
                <Option :value="item.ID" v-for="(item, index) in mrsList" :key="index"
                  @click.native="chooseBeautician(item.Name,item.ID)">{{item.Name}}</Option>
              </Select>
            </FormItem>
          ...
          <i-col span="6">
            <FormItem label="开单时间：" prop="dataTime">
              <DatePicker class="DatePicker_time" type="date" v-model="formValidate.openOrderDate"
                :options="optionsDate" placeholder="开始日期"></DatePicker>
            </FormItem>
          </i-col>
      ...
      <div class="pannel-title">
        ...
        <div>
          <i-button type="text" v-if="xm" @click="showSmallModal(1)" size="small" style="width: auto; height: auto;">
            添加项目</i-button>
          <i-button type="text" v-if='cp' @click="showSmallModal(0)" size="small" style="width: auto; height: auto;">
            添加物资</i-button>
        </div>
      </div>
      <!-- 添加、修改 -->
      ...
      <Input v-model="formValidate.Remark" placeholder="无" type="textarea" :rows="4" readonly v-if="mode == 2" />
      <Input v-model="formValidate.Remark" placeholder="备注" type="textarea" :rows="4" v-else />
      <!-- ** -->
    ...
    <Modal :mask-closable="false" v-model="modal2" width="800" class="dialog" v-if="showAdd || showEdit">
      ...
      <Input v-model="searchKeyWord" placeholder="请输入项目名称" style="width: 300px; margin: 2px 0 2px 0;" icon="search" />

      <div class="buttongroup" v-if="btnYC">
        <Button type="info" @click="quanbu" style="width:50px;padding:0">全部</Button>
        <Button type="info" @click="heitou" style="width:50px;padding:0">黑头</Button>
        <Button type="info" @click="qudou" style="width:50px;padding:0">祛痘</Button>
        <Button type="info" @click="tiyan" style="width:50px;padding:0">体验</Button>
        <Button type="info" @click="huoyang" style="width:50px;padding:0">活氧</Button>
        <Button type="info" @click="nianka" style="width:50px;padding:0">年卡</Button>
        <Button type="info" @click="xiufu" style="width:50px;padding:0">修复</Button>
      </div>

      <div class="table-box">
        <!-- 物资 -->
        <i-table stripe :columns="materialsColumn" :data="setMaterialtData" @on-selection-change="selectMaterial"
          :height="500" v-if="tabIndex == 0"></i-table>
        <!-- 项目 -->
        <i-table stripe :columns="projectColumn" :data="setProjectData" @on-selection-change="selectMaterial"
          :height="500" v-else></i-table>
      </div>
      <div slot="footer" class="footer not-print">
    ...
    <Modal :mask-closable="false" v-model="poupSearchTel" class="dialog" v-if="showAdd">
      ...
      <div slot='footer' class="footer">
        <i-button type="primary" class="btn" @click="searchCustomerTel('xm')" :loading="onLoading">项目</i-button>
        <i-button type="primary" class="btn" @click="searchCustomerTel('cp')" :loading="onLoading">产品</i-button>
  ...
  export default {
      ...
      return {
        ...
        columns1: [
          {
            ...
            align: "center",
            render: (h, params) => {
              return echartsCommon.changeTimeHms(params.row.Date);
            }
          },
          {
            title: "客户姓名",
          ...
          },
          {
            ...
            align: "center",
            render: (h, params) => {
              if (params.row.ChargeDate == null) {
                return ''
              } else {
                return echartsCommon.changeTimeHms(params.row.ChargeDate);
              }
        ...
        choosedItemColumns: [
          ...
          },
          {
            ...
            render: (h, params) => {
              ...
              if (
                params.row.type === "xm" &&
                params.row.canChangePrice === false
              ) {
              ...
              return h("div", [
                h("Input-number", {
                  props: {
                    //readonly:(params.row.type === "xm"?params.row.canChangePrice:false),
                    placeholder: "填写单价",
                    value: Number(params.row.price),
                    max:
                      params.row.type == "wz" ? 999999999 : params.row.highPrice,
                    min:
                      params.row.type == "wz"
                        ? Number(params.row.oldPrice)
                        : params.row.lowPrice
                  },
                  style: {
                    marginRight: "8px"
          ...
          },
          {
            ...
            render: (h, params) => {
              let that = this;
              if (params.row.type === "xm" && params.row.limitSconto === false) {
                return h("div", {}, params.row.rate);
              }
              ...
              return h("div", [
                h("Input-number", {
                  props: {
                    ...
                    // value: params.row.sum,
                    max:
                      params.row.price * params.row.num - params.row.ReilefMoney,
                    min: 0
                  },
                  on: {
                    "on-change": function (e) {
            ...
            render: (h, params) => {
              ...
              return h("div", [
                h("Input-number", {
                  ...
                  props: {
                    placeholder: "减免金额",
                    value: that.choosedItemList[params.index].ReilefMoney,
                    max: 9999999,
                    min: 0
                  },
                  on: {
        ...
        choosedItemLookColumns: [
          ...
          },
          {
            ...
            align: "center"
            // render: (h, params) => {
            //   return h("div", {}, params.row.ReilefMoney.toFixed(2));
            // }
          }
        ]
      };
    ...
    methods: {
      ...
      setData(res) {
        ...
        //that.formValidate.openOrderDate =echartsCommon.changeTimeHms(new Date());
        that.formValidate.openOrderDate = new Date();
      },
      searchReset() {
        let that = this;
        that.$refs.searchTelForm.resetFields();
      ...
      getDetail(id) {
        ...
        api.getConsumptions(id).then(res => {
          that.$store.commit("changeLoadingPage");
          if (res.error_code === "Success") {
            // 设置数据
            if (res.data.ConsumptionDetails[0].GoodsID) {
              that.cp = true;
              that.xm = false;
            } else {
              that.xm = true;
            ...
            if (that.setModelTitle == "浏览") {
              that.xm = false;
              that.cp = false;
            }
            // console.log(res.data.ConsumptionDetails[0].GoodsID);
      ...
      setDetail(res) {
        ...
        that.formValidate.openOrderDate = echartsCommon.changeTimeHms(
          res.InfoDate
        );
        ...
        for (let i in res.ConsumptionDetails) {
          ...
          let a = {
            ...
            rate: dom.Sconto,
            ReilefMoney: dom.ReilefMoney,
            num: dom.Quantity,
            price: dom.Price,
            oldPrice: dom.Price,
            lowPrice: dom.MinPrice,
            highPrice: dom.MaxPrice,
            gg: dom.Standard,
            projectName: dom.Name,
            Name: dom.Name,
            TopCategoryID: dom.TopCategoryID,
            type: "",
            canChangePrice: dom.ChangePrice,
            limitSconto: dom.LimitSconto
          };
          if (dom.ItemID) {
            // 项目
            a.id = dom.ItemID;
      ...
      setUploadData() {
        ...
        let data = {
          ...
          InfoDate: echartsCommon.changeTimeHms(that.formValidate.openOrderDate),
          ConsumptionDetails: []
        };
        // 添加物资记录
        for (let i of that.choosedItemList) {
          data.ConsumptionTotal += i.sum - i.ReilefMoney;
          let a = {
            Name: i.Name,
            ...
            Sconto: i.rate,
            Amount: Number(i.sum),
            ReilefMoney: i.ReilefMoney,
            Remark: "",
            TopCategoryID: i.TopCategoryID
          };
          if (i.type == "wz") {
            a.GoodsID = i.id;
          }
          ...
          if (validate) {
            ...
            api.consumptions(that.setUploadData()).then(response => {
              ...
              if (response.error_code === "Success") {
                ...
                that.$route.params.phone = ""; //项目确诊单的主界面页签存在点击时会触发上一次快捷打开的客户电话信息,造成点击页签会自动出现上一次快捷客户的资料。(已修改)
                that.resetForm();
                that.getList();
              } else {
                that.$Message.error(response.error_message);
        ...
        if (that.tabIndex === 0) {
          ...
          for (let i in that.selection) {
            let a = {
              ...
              TopCategoryID: that.selection[i].TopCategoryID,
              Name: that.selection[i].Name,
              type: "xm",
              limitSconto: that.selection[i].LimitSconto //是否允许折扣
            };
            that.choosedItemList.push(a);
          }
        }
      ...
      calSumMoney() {
        ...
        if (that.choosedItemList.length > 0) {
          for (let i of that.choosedItemList) {
            sum += i.sum;
            calSum1 += i.ReilefMoney;
          }
        }
        that.calSum = sum - calSum1;
      },
      formatData(num) {
        // 四舍五入数据
        let that = this;
    ...
    activated() {
      ...
      that.$nextTick(() => {
        ...
        if (phone) {
          ...
          that.poupSearchTel = true;
          // that.$store.commit('changeLoadingPage')
          //2020年1月7日12:54:47  注释 不能自动去查询手机号，这样总监助理没办法选择开什么单子
          // setTimeout(() => {
          //   that.searchCustomerTel();
          // }, 300);
        }
