    ...
    <Modal :mask-closable="false" v-model="modal1" class="dialog" v-if="showAdd" width="1200">
      ...
      <Form ref="formValidate1" :model="formValidate" :label-width="120" @submit.native.prevent class="form">
        <row type="flex" justify="space-between">
          ...
          <i-col span="5">
            <FormItem prop="OldEmployee" label="原护理师：" :rules="formValidateRule.OldEmployee">
              <Input v-model="formValidate.OldEmployee" disabled></Input>
            </FormItem>
          </i-col>
          <i-col span="5">
            <FormItem prop="NewEmployee" label="转交护理师：" :rules="formValidateRule.NewEmployee">
              ...
              <Input v-model="formValidate.NewEmployee" disabled></Input>
            </FormItem>
          </i-col>
...
export default {
      ...
      columns1: [
        {
          ...
          align: "center",
          render: (h, params) => {
            if (!params.row.DeliverTime) {
              return "";
            }
            return h(
              "div",
              {},
              moment(params.row.DeliverTime).format("YYYY-MM-DD HH:mm:ss")
            );
          }
        },
        {
        ...
        {
          title: "转交到的护理师",
          key: "DeliverToBeauticianName",
          align: "center"
        },
        {
          title: "经办人",
          key: "ModifyEmployeeName",
          align: "center"
        },
        {
          title: "审核状态",
  ...
  methods: {
    ...
    setData(res) {
      ...
      that.formValidate.OldEmployee = res.OldEmployee;
      that.formValidate.NewEmployee = res.NewEmployee;
    ...
    searchCustomerTel() {
      ...
      that.loading = true;
      api.getCustomerDetail2(data).then(response => {
        let a = {
          Name: "",
          Age: "",
      ...
      that.$refs.formValidate1.validate(validate => {
        if (validate) {
          ...
          let data = {
            ...
            AuditNotes: that.formValidate.Remark,
            Id: that.ID,
            CustomerId: that.currentID,
            Status: that.status
          };
          api.ChangePractitioner(data).then(response => {
            that.loading = false;
    ...
    GetPractitionerList() {
      ...
      manager.GetPractitionerList(data).then(response => {
        if (response.error_code === "Success") {
