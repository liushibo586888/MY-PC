<template>
  <div class="sf-container container">
        ...
        <row :gutter="20">
          ...
          <i-col span="12">
            <formItem class="btn-box">
              ...
              <i-button type="info" class="btn" @click="shenhe" v-if="isShow" :disabled="!rowID">审核</i-button>
              <!--<Button type="warning" class="btn btn-add" @click="checkBillStatus" v-if="showAdd" :disabled="!rowID">收费</Button> -->
              <Button type="warning" class="btn btn-add" @click="CheckCustArrDetail" v-if="showAdd" :disabled="!rowID">收费</Button>
    ...
    <!--点击收费时 先查询是否有欠费的信息提示框-->
    <Modal v-model="ArrModal" :title="ModaluserName" :closable="false" width='60%' :mask-closable="false" ok-text="欠款回收" cancel-text="继续收费" @on-ok="tabsOnOk" @on-cancel="tabsOnCancle">
      <Table :columns="ArrColumns" :data="CustArrDetail"></Table>
    </Modal>
    ...
    <Modal :mask-closable="false" v-model="modal1" class="dialog" width="1200" v-if="showAdd || showEdit">
        ...
        <row type="flex" justify="space-between">
          <i-col span="6">
            <FormItem prop="CustomerPhone" label="电话：">
              <Input v-model="formValidate.CustomerPhone" placeholder="无" :readonly="true" />
            </FormItem>
          </i-col>
          <i-col span="6">
            <FormItem prop="CustomerName" label="姓名：">
              <Input v-model="formValidate.CustomerName" placeholder="无" :readonly="true" />
            </FormItem>
          </i-col>
          <i-col span="6">
            <FormItem label="年龄：" prop="CustomerAge">
              <Input v-model="formValidate.CustomerAge" placeholder="无" :readonly="true" />
            </FormItem>
          </i-col>
          <i-col span="6">
            <FormItem prop="CustomerSex" label="性别：">
              <Input v-model="formValidate.CustomerSex" placeholder="无" :readonly="true" />
            </FormItem>
          </i-col>
        </row>
        <row type="flex" justify="space-between">
          <i-col span="6">
            <FormItem label="预交余额：" prop="CashBalance">
              <InputNumber v-model="formValidate.CashBalance" placeholder="无" :readonly="true" :precision="2" style="width:100%;"></InputNumber>
            </FormItem>
          </i-col>
          <i-col span="6">
            <FormItem label="积分余额：" prop="IntegrationBalance">
              <InputNumber v-model="formValidate.IntegrationBalance" placeholder="无" :readonly="true" :precision="2" style="width:100%;"></InputNumber>
            </FormItem>
          </i-col>
          <i-col span="6">
            <FormItem label="欠款余额：" prop="ArrearsBalance">
              <InputNumber v-model="formValidate.ArrearsBalance" placeholder="无" :readonly="true" :precision="2" style="width:100%;"></InputNumber>
            </FormItem>
          </i-col>
          <i-col span="6">
            <FormItem label="赠送余额：" prop="HandselBalance">
              <InputNumber v-model="formValidate.HandselBalance" placeholder="无" :readonly="true" :precision="2" style="width:100%;"></InputNumber>
            </FormItem>
          </i-col>
        </row>
        <row type="flex">
          <i-col span="6">
            <FormItem label="美肤总监：" prop="SceneEmployeeName">
              <Input v-model="formValidate.SceneEmployeeName" placeholder="无" :readonly="true" />
            </FormItem>
          </i-col>
          <i-col span="6">
            <FormItem label="渠道咨询：" prop="TMKEmployeeName">
              <Input v-model="formValidate.TMKEmployeeName" placeholder="无" :readonly="true" />
            </FormItem>
          </i-col>
        </row>
        <!-- ** -->
      </Form>
      <!-- 确诊单明细 不可修改 :height="300"-->
      <div class="table-box">
        <i-table stripe :columns="detailHead1" :data="detailData"></i-table>
      </div>
      <!-- ** -->
      <!-- 收费表格 -->
      <!-- 收费表格 浏览用 :height="200"-->
      ...
      <Input v-model="formValidate.Remark" placeholder="备注" type="textarea" :rows="4" style="margin-top:20px;" v-else readonly />
      <!-- ** -->
    ...
    <!-- 审核提示 -->
    <Modal :mask-closable="false" v-model="SHshow" title="提示" @on-ok="shenHeModal">
      <p style="text-align:center;font-weight:900;font-size:15px"> 确认审核吗？</p>
...
export default {
  data() {
    return {
      ...
      RefundPower: false, //判断是否有退费的权限
      ConsumptionStatus: 0, //当前选择的客户收费状态

      ModaluserName: null, //收费时显示那个客户有欠费
      ArrModal: false, //收费时给前台提示这个客户有欠费
      CustArrDetail: [], //客户项目欠款信息
      AuditRelief: "",
      shenheYJ: "",
      SHshow: false,
      columnColor: "",
      ...
      ArrColumns: [
        ...
        },
        {
          ...
          key: "changeDate",
          render: (h, parmas) => {
            return echartsCommon.DATEsfm(h, parmas.row.changeDate);
          }
        },
        {
          title: "项目名称",
        ...
        },
        {
          ...
          key: "Surplus",
          render: (h, params) => {
            return params.row.changeAmount - params.row.ArrAmount;
          }
        },
        {
          title: "护理师",
      ...
      columns1: [
        ...
        },
        {
          ...
          align: "center",
          render: (h, parmas) => {
            if (parmas.row.Status == 1 && this.RefundPower) {
              return h("div", [
                h(
                  "Button",
                  {
                    ...
                    on: {
                      click: () => {
                        ...
                        //parmas.row.CustomerID
                        this.btnRefundMoney(
                          parmas.row.CustomerPhone,
                          parmas.row.ID
                        );
                      }
                    }
            ...
            //              this.btnRefundMoney(parmas.row.CustomerPhone);
            //               //this.checkBillStatus(parmas.row)ID
            //           }
            //       }
            //   },"退费")
            //  }
  ...
  methods: {
    ...
    shenhe() {
      let that = this;
      if (that.AuditRelief == 0) {
        that.$Message.error("该收费单无需审核!");
        return false;
      }
      that.SHshow = true;
    },
    ...
    shenHeModal() {
      ...
      let data = {
        ...
        ConsumptionInfoId: that.rowID,
        Remark: that.shenheYJ
      };
      api.AuditRelief(data).then(response => {
    ...
    getRowID(row) {
      ...
      this.ModaluserName = "客户 " + row.CustomerName + " 的项目欠款信息";
      let that = this;
      that.rowID = row.ID;
      that.AuditRelief = row.AuditRelief;
      that.rowPhone = row.CustomerPhone;
      that.CustId = row.CustomerID;
    },
    handleSpinShow() {
      // 加载框
      let that = this;
    ...
    //点击退费时跳转到退费页面
    btnRefundMoney(CustPhone, ConsumptionInfoId) {
      this.$router.push({
        path: "/charge/tf",
        query: {
          CustPhone: CustPhone,
          ConsumptionInfoId: ConsumptionInfoId
        }
      });
    },
    tabsOnOk() {
      this.$router.push({
        path: "/charge/arrearsRecovery",
        query: {
          CustPhone: this.rowPhone
        }
      });
    },
    tabsOnCancle() {
    ...
    CheckCustArrDetail() {
      //收费之前先判断当前客户是否有欠费的项目 2019年12月4日11:36:33 ly
      let that = this;
      that.resetForm();
      if (!that.rowID) {
      ...
      let pam = {
        custId: this.CustId,
        consumptionID: this.rowID
      };
      //未收费的情况
      if (this.ConsumptionStatus != 1) {
        api.isCustArrDetail(pam).then(res => {
          if ((res.error_code = "Success" && res.data.length > 0)) {
            this.ArrModal = true;
            this.CustArrDetail = res.data;
          } else if ((res.error_code = "Success" && res.data.length == 0)) {
            this.checkBillStatus();
          } else {
            that.$Message.info({
    ...
    checkBillStatus() {
      ...
      that.$store.commit("changeLoadingPage");
      that.resetForm();
      api.isAllowCharge(data).then(res => {
        // 可以收费 添加
        if (res.error_code === "Success") {
          that.alreadyPaid = false;
          // that.searchCustomerTel(that.rowPhone)
          api.seeCharge(that.rowID).then(res => {
            that.$store.commit("changeLoadingPage");
            if (res.error_code === "Success") {
              that.CustomerOpenID = res.data.CustomerOpenID;
              that.HLSOpenID = res.data.BeauticianOpenID;
              that.khName = res.data.CustomerName;
              that.storeName = res.data.StoreName;
              that.kdtime = res.data.InfoDate.replace("T", " ");
          ...
          let content = "";
          // 不可收费
          if (that.AuditRelief == 1) {
            content = "该订单有减免金额，请审核";
          } else {
            content = "该订单已收费不可再收费";
          }
          that.$store.commit("changeLoadingPage");
    ...
    setData(res) {
      ...
      that.formValidate.CustomerPhone =
        res.CustomerPhone.substr(0, 3) +
        "****" +
        res.CustomerPhone.substr(7, 11);
    ...
    addForm() {
      ...
      for (let i in that.detailData) {
        sum += that.detailData[i].Amount - that.detailData[i].ReilefMoney;
      }
      // 计算已支付
      let paid = 0;
      for (let i in that.chargeTableData) {
      ...
      this.$Modal.confirm({
        ...
        onOk: () => {
          if (!lock) {
            lock = true;
            if (sum == paid) {
              that.uploadCharge(paid.toFixed(2));
            } else {
              that.$Message.info({
                content: "金额错误请重新检查",
                duration: 10
    ...
    uploadCharge(paid) {
      ...
      api.addCharge(data).then(response => {
        ...
        if (response.error_code === "Success") {
          if (that.CustomerOpenID != null) {
            //发消息给客户
            that.checkCustomerOpenID(
              that.CustomerOpenID,
              that.kdtime,
              paid,
              that.storeName
          ...
          if (that.HLSOpenID != null) {
            //发消息给护理师
            that.checkHLSOpenID(that.HLSOpenID, that.kdtime, that.khName);
          }
    ...
    //发消息给顾客
    checkCustomerOpenID(CustomerOpenID, time, paid, store) {
      let that = this;
      var data = {
        touser: CustomerOpenID,
        template_id: echartsCommon.CustomerOpenIDsf(),
        //服务通知中进入小程序的入口
      ...
      let jd = {
        ...
        employeeId: that.userMes.EmployeeID,
        employeeName: that.userMes.EmployeeName,
        CustomerID: that.CustId,
        Rmk: "web收费消息"
      };
      api.SendAppletNotice(jd).then(res => {
        // console.log(res);
        if (res.error_code === "Success") {
          that.$Message.success("顾客消息通知发送成功");
        } else {
          that.$Message.error(res.error_message);
        }
      });
    ...
    //发消息给护理师
    checkHLSOpenID(HLSOpenID, time, khName) {
      let that = this;
      var data = {
        touser: HLSOpenID,
        template_id: echartsCommon.HLSOpenIDsf(),
        //服务通知中进入小程序的入口
      ...
      api.SendToEmployee(jd).then(res => {
        // console.log(res);
        if (res.error_code === "Success") {
          that.$Message.success("护理师消息通知发送成功");
        } else {
          that.$Message.error(res.error_message);
        }
      });
      ...
      api.chargeGetList(data).then(response => {
        if (response.error_code === "Success") {
          ...
          for (let i of res.list) {
            if (i.ReliefMoney > 0 && i.AuditRelief == 1) {
              i["cellClassName"] = { ReliefMoney: "demo-table-info-cell-name" };
            }
          }
          that.list = res.list;
  ...
  mounted() {
    ...
    that.$nextTick(() => {
      ...
      that.RefundPower = btnQX.apiUrlList("/Refunds/Add"); //接收是否有退费权限
    });
  },
  filters: {
    formatMoney(val) {
