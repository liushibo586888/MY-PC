<template>
  <div class="qdxd-container container">
        ...
        <row :gutter="20">
          ...
          <i-col span="3">
            <formItem prop="startDate">
              <DatePicker class="DatePicker_time" type="date" :options="options1" placeholder="开始日期" :clearable="false"
                v-model="formItem.startDate" :editable="false"></DatePicker>
            </formItem>
          </i-col>
          <i-col span="3">
          ...
          <i-col span="12">
            <formItem class="btn-box">
              ...
              <Dropdown transfer @on-click="addPoups" v-if="shortcutMenuIndex > 0">
                <Button type="primary" class="btn btn-add" :disabled="!rowPhone">快捷菜单</Button>
    ...
    <!-- 查询电话号码弹窗 -->
    <Modal :mask-closable="false" v-model="poupSearchTel" :scrollable="true" class="dialog" v-if="showAdd">
      <div slot="header" class="header not-print" v-if="tabName=='tabTel'">请输入电话号码</div>
      <div slot="header" class="header not-print" v-else>请输入预约码</div>
      <div slot="close" class="close not-print">
        <i class="ivu-icon ivu-icon-aaa-guanbi" @click="searchReset"></i>
      </div>
      ...
      <Form :model="searchTel" ref="searchTelForm" @submit.native.prevent="searchCustomerTel">
        <Tabs v-model="tabName" @on-click="tabClick">
          <TabPane label="常规到店" name="tabTel">
          ...
          <TabPane label="预约码到店" name="tabYqm">
            <Form-item prop="yqm" :rules="searchRule.yqm">
              <i-input v-model="searchTel.yqm" placeholder="请输入六位数的预约码"></i-input>
            </Form-item>
          </TabPane>
        </Tabs>
      </Form>
    ...
    <!-- 弹窗 -->
    <Modal :mask-closable="false" v-model="modal1" class="dialog" v-if="showAdd" :styles="{top: '10px'}" width="1000">
      <div slot="header" class="header">新增</div>
      <Form ref="formValidate1" :model="formValidate" :label-width="84" @submit.native.prevent class="form">
        <row type="flex" justify="space-between">
          <i-col span="11">
            <FormItem label="客户姓名：" prop="CustomerName" :rules="formValidateRule.CustomerName">
          ...
          <i-col span="11">
            <FormItem label="身份证：" prop="IDCardNumber">
              <Input v-model="formValidate.IDCardNumber" placeholder="身份证" :disabled="customerEnable"
                @on-blur="handleInputIDcard(formValidate.IDCardNumber)" />
            </FormItem>
          </i-col>
        </row>
        ...
        <row type="flex" justify="space-between">
          ...
          <i-col span="11">
            <FormItem label="生日：" prop="Birthday" :rules="formValidateRule.Birthday">
              <DatePicker @on-change="dianji1" style="width:100%" type="date" placeholder="客户生日"
                v-model="formValidate.Birthday" :editable="false" :readonly="birthdayEnable"></DatePicker>
            </FormItem>
          </i-col>
        </row>
        ...
        <row type="flex" justify="space-between">
          <i-col span="11">
            <FormItem label="年龄：" prop="CustomerAge">
              <!-- v-if="customerEnable" -->
              <Input-number v-model="formValidate.CustomerAge" placeholder="年龄：" :disabled="true" style="width:100%">
              </Input-number>
            </FormItem>
        ...
        <row type="flex" justify="space-between">
          <i-col span="11">
            <FormItem label="门店总监：" prop="SceneEmployeeID" :rules="formValidateRule.SceneEmployeeID">
              <Select v-model="formValidate.SceneEmployeeID" :filterable="true">
                <Option :value="item.ID" v-for="(item, index) in mfzjList" :key="index">{{item.Name}}</Option>
              </Select>
            </FormItem>
          </i-col>
          <!-- <i-col span="11">
            <FormItem label="客户状态：" prop="CustomerStatus" :rules="formValidateRule.CustomerStatus">
              <Select v-model="formValidate.CustomerStatus" :filterable="true">
                <Option :value="item.ID" v-for="(item, index) in CustomerStatusList" :key="index">{{item.Name}}</Option>
              </Select>
            </FormItem>
          </i-col> -->
          <i-col span="11">
            <FormItem label="来源渠道：" prop="ChannelName" :rules="formValidateRule.ChannelName"
              v-if="customerEnable && formValidate.ChannelName">
        ...
        <row type="flex" justify="space-between">
          ...
          <i-col span="11">
            <FormItem label="咨询项目：" prop="ItemCategoryID" :rules="formValidateRule.ItemCategoryID">
              <Select v-model="formValidate.ItemCategoryID">
                <Option :value="item.ID" v-for="(item, index) in zxxmList" :key="index"
                  @click.native="chooseYymd(index)">{{item.Name}}</Option>
              </Select>
            </FormItem>
          </i-col>
        </row>
        ...
        <row type="flex" justify="space-between">
          ...
          <i-col span="11">
            <FormItem prop="ItemCategoryMXID" :rules="formValidateRule.ItemCategoryMXID">
              <Select v-model="formValidate.ItemCategoryMXID">
                <Option :value="item.ID" v-for="(item, index) in zxxmMXList" :key="index"
                  @click.native="chooseYymdMX(item.ID)">{{item.Name}}</Option>
              </Select>
            </FormItem>
          </i-col>
        </row>
  ...
  export default {
    ...
    data() {
      ...
      const validateCode = (rule, value, callback) => {
        let reg = /^\d{6}$/;
        if (value === "") {
          callback(new Error("预约码不得为空"));
        } else if (!reg.test(value)) {
          callback(new Error("请输入正确的六位数预约吗"));
        } else {
          callback();
        }
      };
    ...
    methods: {
      ...
      handleInputIDcard(e) {
        let that = this;
        that.formValidate.IDCardNumber = echartsCommon.IDcard(that.$Message, e);
      },
      tiaozhuan() {
        let that = this;
        that.$router.push({
      ...
      chooseYymd(index) {
        let that = this;
        that.zxxmMXList = [];
        that.zxxmMXList = JSON.parse(
          JSON.stringify(that.zxxmList[index].ListItem)
        );
      },
      chooseYymdMX(id) {
        let that = this;
        that.formValidate.ItemCategoryMXID = id;
      },
      resetSearch() {
        let that = this;
        ...
        that.$refs.formValidate1.validate(validate => {
          if (validate) {
            ...
            let data = {
              ...
              ItemCategoryID: a.ItemCategoryMXID,
            ...
            api.customerComeLogs(data).then(response => {
              ...
              if (response.error_code === "Success") {
                ...
                //   that.checkCustomerOpenID(that.CustomerOpenID);
                // }
                that.$refs.formValidate1.resetFields();
                that.modal1 = false;
                that.getList();
      ...
      checkCustomerOpenID(CustomerOpenID) {
        let that = this;
        var data = {
          touser: CustomerOpenID,
          template_id: echartsCommon.CustomerOpenIDdddj(),
          //服务通知中进入小程序的入口
      ...
      searchCustomerTel() {
        ...
        let reg = /^1\d{10}$/;
        if (
          (that.tabName == "tabTel" && that.searchTel.tel == "") ||
          (that.tabName == "tabTel" && !reg.test(that.searchTel.tel))
        ) {
          that.$Message.error({
            content: "请输入用户的联系电话",
            duration: 3
          ...
          (that.tabName == "tabYqm" && that.searchTel.yqm == "") ||
          (that.tabName == "tabYqm" && that.searchTel.yqm.length != 6)
        ) {
          that.$Message.error({
            content: "请输入正确六位数的的预约码",
            duration: 3
          });
          return;
        }
        ...
        let data = {
          ...
          customerCode: that.searchTel.yqm,
          EmployeeID: that.userMes.EmployeeID
        };
        that.loading = true;
        api.getCustomerByPhoneCode(data).then(response => {
          let a = {
            Name: "",
            Age: "",
          ...
          if (response.error_code === "Success") {
            ...
            that.CustomerOpenID = response.data.CustomerOpenID;
      ...
      setData(res) {
        ...
        that.formValidate.CustomerAge = res.Age ? res.Age : 0;
        ...
        let month = moment(new Date()).format("MM-DD");
        that.formValidate.Birthday = res.Birthday
          ? res.Birthday
          : year - 18 + "-" + month;
      },
      searchReset() {
