    ...
    <Modal :mask-closable="false" v-model="modal1" class="dialog" width="1000" class-name="vertical-center-modal" v-if="showAdd || showLookUp">
      ...
      <Form ref="formValidate1" :model="formValidate" :label-width="120" @submit.native.prevent class="form">
        ...
        <row type="flex" justify="start">
          ...
          <formItem label="退预交金日期：" prop="startDate">
  ...
  methods: {
    ...
    addForm() {
      ...
      let data = {
        ...
        HandlingFee: 0,
        Came: 1,
        Date: echartsCommon.changeTimeHms(that.CZDate),
    ...
    setData(res) {
      ...
      that.formValidate.BeauticianID = res.BeauticianEmployee;
    ...
    getEmployeesList() {
      ...
      api.QueryCustPrepayment(data).then(response => {
        if (response.error_code === "Success") {
          let res = response.data;
          that.HLSList = res;
        } else {
