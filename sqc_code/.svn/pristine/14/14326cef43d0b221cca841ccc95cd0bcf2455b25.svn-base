<template>
  <div class="yggl-container container">
    <div class="search-box">
      <Form :model="formItem" :label-width="0" inline @submit.native.prevent class="search-form" ref="searchForm">
        <row :gutter="20">
          ...
          <i-col span="3">
            <FormItem prop="SSQUID">
              <Select v-model="formItem.SSQUID" placeholder="区域" :filterable="true">
                <Option value="all" @click.native="SSQYchoose()">所有门店</Option>
                <Option v-for="item in SouSQYList" :value="item.Name" :key="item.ID" @click.native="SSQYchoose(item.Name,item.ID)">{{ item.Name }}</Option>
              </Select>
            </FormItem>
          </i-col>
          <i-col v-show="isSee" span="3">
            <FormItem prop="SSMDID">
              <Select v-model="formItem.SSMDID" placeholder="门店" :filterable="true">
                <Option v-for="item in SouSMDList" :value="item.Name" :key="item.ID" @click.native="SSMDchoose(item.Name,item.ID)">{{ item.Name }}</Option>
              </Select>
            </FormItem>
          </i-col>
          <i-col span="3">
            <FormItem prop="IFYX">
              <Select v-model="formItem.IFYX" placeholder="是否有效" :filterable="true">
    ...
    <!-- 转交弹框 -->
    <Modal v-model="modalZj" :closable='false' :mask-closable='false' title="转交客户名单" width="800" :styles="{top: '20px',paddingLeft:'20px'}">
      ...
      <Form ref="formValidate1" :model="formValidate" :label-width="120" @submit.native.prevent class="form">
        <row type="flex" justify="space-between">
          <i-col span="11">
            <FormItem prop="ZJHLSID" label="选择转接护理师：" :rules="formValidateRule.ZJHLSID">
    ...
    <!-- 新增弹窗 -->
    <Modal :mask-closable="false" v-model="modal1" :styles="{top: '20px'}" class="dialog" :width="1000" v-if="setAuthorAdd || setAuthorEdit">
  ...
  methods: {
    ...
    SSQYchoose(name, id) {
      ...
      if (name === undefined && id === undefined) {
        that.isSee = false; //隐藏门店选择框
        that.formItem.SSQYname = "所有";
        that.formItem.SSQYID = null;
        that.SouSMDList = [];
      } else {
    ...
    getList() {
      ...
      // 2019年12月15日10:37:56 ly
      if (that.formItem.SSQUID != "" && that.formItem.SSQUID != "all") {
        if (that.formItem.SSMDID == "") {
          for (const i in that.SouSMDList) {
            data.StoreID.push(that.SouSMDList[i].ID);
          }
        } else {
          data.StoreID.push(that.formItem.SSMDID);
        }
      }
    ...
    getCity() {
      let that = this;
      let data = {
        sCity: that.formItem.SSQYID,
        employeeId: that.userMes.EmployeeID
      };
      api.baogetEmployeeByAllStoreInfo(data).then(response => {
        if (response.error_code === "Success") {
          that.SouSMDList = response.data.list;
