...
export default {
  ...
  methods: {
    chooseTerrace(id, name) {
      ...
      that.roleDetailsTemp=[];
      for (let i of that.roleDetails) {
        if(i.PlatformType===id){
          that.roleDetailsTemp.push(i);
        }
      }
    },
    showAddPoup() {
    ...
    setRole(){
      let that=this;
      that.roleDetailsTemp=[];//清空权限
      let platformTypeId=that.formItem.PlatformType
      for (let i of that.roleDetails) {
        if(i.PlatformType===platformTypeId){
          that.roleDetailsTemp.push(i);
        }
      }
    },
    ...
    setData() {
      ...
      let data = {
        ...
        DefaultControllerCategoryID:that.formValidate.DefaultControllerCategoryID,
        RoleDetails: [],
        PlatformType:that.formItem.PlatformType
      };
      for (let i of that.authorityList) {
        for (let j of that.roleDetailsTemp) {
          if (i === j.ActionID) {
            j.IsAllow = true;
          }
        }
      }
      data.RoleDetails = that.roleDetailsTemp;
      return data;
    },
    initTableHeight() {
      let that = this;
    ...
    getDropdownListData() {
      ...
      superManager.getDropdownListData().then(response => {
        if (response.error_code === "Success") {
          that.defaultPoupList = [];
          let arr = response.data.list;
          let arr1 = [];
          for (let i = 0; i < arr.length; i++) {
            if (arr[i].PlatformType == that.formItem.PlatformType) {
              that.defaultPoupList.push(arr[i]);
            }
          }
        } else {
          that.$Message.error(response.error_message);
