<template>
    <div class="xmqz-container container">
        ...
        <!-- 弹窗 -->
        <Modal :mask-closable="false" v-model="modal1" width="1400" :scrollable="true" class="dialog" :styles="{top: '20px'}">
                ...
                <row type="flex" justify="space-between">
                    <i-col span="24">
                        ...
                        <Steps :current="Num">
                            ...
                            <Step v-for="(item, index) in SHpeoples" :key="index" :title="item.zt" :content="item.name"></Step>
                        </Steps>
                        <!-- </div> -->

                    </i-col>
                    <!-- <i-col span="2">
                        <button @click="TJSHren">添加审核人</button>
        ...
        <!-- 选择审核人 -->
        <Modal :mask-closable="false" v-model="modalFQ" title="请选择审核人" @on-ok="okFQ" @on-cancel="cancelFQ">
            <Button style="margin-right:10px" v-for="(item,index) in XZSHren" :key="index" :class="item.checked?'checked':''" @click="XZbtnClick(index)">
                {{item.Name}}
            </Button>
        </Modal>
...
export default {
  data() {
    return {
      XZSHren: [
        { id: "0", Name: "bobo", zt: "待审核", checked: false },
        { id: "1", Name: "婷婷", zt: "待审核", checked: false }
      ],
      Num:0,
      modalFQ: false,
      FQRname: "",
  ...
  methods: {
    XZbtnClick(e) {
      let arr = this.XZSHren;
      arr[e].checked = !arr[e].checked;
      this.XZSHren = arr;
      //   console.log(this.XZSHren);
    },
    TJSHren() {
      let that = this;
      that.modalFQ = true;
  ...
  mounted() {
    let that = this;
    that.$nextTick(() => {
      that.FQRid = that.userMes.EmployeeID;
