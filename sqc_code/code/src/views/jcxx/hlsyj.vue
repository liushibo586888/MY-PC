<template>
  <div class="zfmx-container container">
    ...
    <div class="table-box" id="tableBox">
      <Table :columns="columns1" :data="list" highlight-row :height="setTableHeight" ref="mainTable" @on-selection-change='selectionChange' @on-row-click="getRowID"></Table>
    ...
    <Modal :mask-closable="false" v-model="modalSH">
      <div slot='close' class="close not-print">
        <i class='ivu-icon ivu-icon-aaa-guanbi' @click='iconcancel'></i>
      </div>
      <div>
        审核意见：
...
export default {
  ...
  methods: {
    selectionChange(e) {
      let that = this;
      that.NursingPerformanceTargetId = [];
      e.forEach(v => {
        that.NursingPerformanceTargetId.push(v.Id);
      });
    },
    //控制输入框只能输入数字
    handleInputYear(e) {
    ...
    ZDshenhe() {
      let that = this;
      if (that.NursingPerformanceTargetId.length == 0) {
        that.$Message.error("请选择审核数据");
        return;
      } else {
        that.modalSH = true;
      }
    ...
    ok() {
      ...
      let data = {
        StoreId: that.formItem.StoreID,
        Status: 2,
        DateTime: moment(that.formItem.StartDate).format("YYYY-MM"),
        EmployeeId: that.userMes.EmployeeID,
        Remark: that.SHjianyi,
        NursingPerformanceTargetId: that.NursingPerformanceTargetId
      };
        ...
        api.NursingPerformanceTargetAuditgetAuditList(data).then(response => {
          if (response.error_code === "Success") {
            ...
            for (let i = 0; i < res.length; i++) {
              res[i].Status = arr[res[i].Status - 1];
            }
          } else {
            that.$Message.error(response.error_message);
          }
    ...
    getList() {
      ...
      let data = {
        ...
        StoreId: that.formItem.StoreID,
        StartDate: moment(that.formItem.StartDate).format("YYYY-MM"),
      ...
      api.NursingPerformanceTarget(data).then(response => {
        if (response.error_code === "Success") {
          ...
          that.page = response.data.Result.page;
          that.tablePage.allNum = response.data.Result.total;
          that.tablePage.maxPageNum = response.data.Result.totalPage;
          that.setPage();
        } else {
          that.$Message.error(response.error_message);
        }
