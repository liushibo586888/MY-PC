<template>
  <div class="khgl-container container">
    ...
    <div class="bottom-box">
      ...
      <div class="row-box">
        {{ tablePage.startNum }} - {{ tablePage.endNum }}条/共{{
          tablePage.allNum
        }}条
      </div>
    ...
    <Modal :mask-closable="false" v-model="modal1" class="dialog" v-if="showAdd">
      ...
      <Form ref="formValidate1" :model="formValidate" :label-width="120" @submit.native.prevent class="form">
        <FormItem prop="CustomerName" label="客户姓名：" :rules="formValidateRule.CustomerName">
          <Input v-model="formValidate.CustomerName" :readonly="true" />
        </FormItem>
        <FormItem prop="CustomerPhone" label="电话：" :rules="formValidateRule.CustomerPhone">
          <Input v-model="formValidate.CustomerPhone" :readonly="true" />
        </FormItem>
        <FormItem label="原所属门店：" prop="oldStore">
          <Input v-model="formValidate.oldStore" :readonly="true" />
        </FormItem>
        <FormItem label="原所属总监：" prop="oldZJ">
          <Input v-model="formValidate.oldZJ" :readonly="true" />
        </FormItem>
        <FormItem label="原所属护理师：" prop="oldHLS">
          <Input v-model="formValidate.oldHLS" :readonly="true" />
        </FormItem>
        <FormItem label="选择区域：" prop="areaID">
          <Select v-model="formValidate.areaID">
            <Option v-for="item in areaList" :value="item.ID" :key="item.ID" @click.native='QYoptionClick(item.ID)'>{{
              item.Name
            }}</Option>
          </Select>
        </FormItem>
        <FormItem label="现所属门店：" prop="newStoreID" :rules="formValidateRule.newStoreID">
          <Select v-model="formValidate.newStoreID">
            <Option v-for="item in storeList" :value="item.ID" :key="item.ID" @click.native='MDoptionClick(item.ID)'>{{
              item.Name
            }}</Option>
          </Select>
        </FormItem>
        <FormItem label="现所属总监：" prop="ZJID" :rules="formValidateRule.ZJID">
          <Select v-model="formValidate.ZJID">
            <Option v-for="item in ZJList" :value="item.ID" :key="item.ID">{{
              item.Name
            }}</Option>
          </Select>
        </FormItem>
        <FormItem label="现所属护理师：" prop="HLSID" :rules="formValidateRule.HLSID">
          <Select v-model="formValidate.HLSID">
            <Option v-for="item in HLSList" :value="item.ID" :key="item.ID">{{
              item.Name
            }}</Option>
          </Select>
        </FormItem>
        <FormItem label="备注：" prop="Remark">
          <Input v-model="formValidate.Remark" placeholder="备注" type="textarea" />
        </FormItem>
      </Form>
...
export default {
  ...
  methods: {
    ...
    setData(res) {
      ...
      that.formValidate.oldZJ = res.SceneEmployeeName;
      that.formValidate.oldHLS = res.BeauticianEmployeeName;
    },
    searchCustomerTel() {
      let that = this;
      ...
      that.$refs.formValidate1.validate(validate => {
        if (validate) {
          ...
          let data = {
            ...
            NewSceneEmployee: that.formValidate.ZJID,
            NewBeauticianEmployee: that.formValidate.HLSID,
            Remark: that.formValidate.Remark
          };
          // that.loading = false;
          // return;
          manager.AddChangeStoreInfo(data).then(response => {
            that.loading = false;
            if (response.error_code === "Success") {
    ...
    QYoptionClick(QYID) {
      let that = this;
      that.getStoreList(QYID).then(() => {
        that.formValidate.newStoreID = "";
      });
    },
    //选择门店
    MDoptionClick(MDID) {
      let that = this;
      that.getZJList(MDID).then(() => {
        that.formValidate.ZJID = "";
      });
      that.getHLSList(MDID).then(() => {
        that.formValidate.HLSID = "";
      });
    },
    // 获取区域列表
    getQYList() {
    ...
    async getStoreList(areaID) {
      let that = this;
      let listCity = [];
      listCity.push(areaID);
      let data = {
        listCity: listCity
      };
      await api.getStoreInfoCity(data).then(res => {
    ...
    // 获取护理师列表
    async getHLSList(MDID) {
      let that = this;
      let data = {
        StoreID: MDID
      };
      await api.GetBeauticiansByStoreID(data).then(res => {
        if (res.error_code === "Success") {
          that.HLSList = res.data.list;
        } else {
          this.$Message.error(res.error_message);
        }
      });
    ...
    // 获取护总监列表
    async getZJList(MDID) {
      let that = this;
      that.ZJList = [];
      let data = {
        StoreID: MDID
      };
      await api.GetSceneEmployeeByStoreID(data).then(res => {
        if (res.error_code === "Success") {
          that.ZJList = res.data.list;
        } else {
          this.$Message.error(res.error_message);
        }
      });
