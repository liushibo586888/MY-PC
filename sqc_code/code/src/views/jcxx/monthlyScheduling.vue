...
export default {
        ...
        },
        {
          ...
          render: (h, params) => {
            ...
            return h(
              "div",
              {},
              moment(params.row.CheckDate).format("YYYY-MM-DD")
            );
          }
        },
        {
  ...
  methods: {
    ...
    searchForm() {
      ...
      let that = this;
            //判断两个时间段大小
      if(echartsCommon.ContrastTime(this.$Message,that.formItem.startDate, that.formItem.endDate))
      {
    ...
    chooseBeautician(item) {
      ...
      scheduling.getValidWorkHoursDefintion(data).then(response => {
        if (response.error_code === "Success") {
          ...
          for (let i of that.scheduleDayInfo[that.nowDay - 1]
            .scheduleDayDetails) {
            if (i.EmployeeID === item.EmployeeID) {
              that.classesIndex = i.WorkHoursID;
              break;
