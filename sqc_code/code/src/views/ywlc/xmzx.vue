<template>
  <div class="sf-container container">
        ...
        <row :gutter="20">
          ...
          <i-col span="11">
            <formItem class="btn-box">
              ...
              <i-button type="primary" class="btn btn-add" @click="checkBillStatus" v-if="setAuthorEdit" :disabled="!rowID ||ExecuteStatus==2||ItemEndModels!=null">执行</i-button>
              <i-button type="error" class="btn" @click="stopZX" v-if="btnIsshow" :disabled="!rowID ||ExecuteStatus==2||ItemEndModels!=null">项目终止</i-button>
            </formItem>
          </i-col>
        </row>
      </Form>
    ...
    <Modal :mask-closable="false" v-model="modal1" class="dialog" width="1200" class-name="vertical-center-modal">
        ...
        <row type="flex" justify="space-between" v-if="QXZX">
          <i-col span="6">
            <FormItem label="项目金额：" prop="ItemTotalAmount">
              <Input v-model="formValidate.ItemTotalAmount" placeholder="项目总金额" :readonly="true" />
            </FormItem>
          </i-col>
          <i-col span="6">
            <FormItem label="消费金额：" prop="SpendAmount">
              <Input v-model="formValidate.SpendAmount" placeholder="消费金额" :readonly="true" />
            </FormItem>
          </i-col>
          <i-col span="6">
            <FormItem label="剩余金额：" prop="ResidueAmount">
              <Input v-model="formValidate.ResidueAmount" placeholder="剩余金额" :readonly="true" />
            </FormItem>
          </i-col>
        </row>
        ...
        <row type="flex" justify="space-between" v-if="QXZX">
          <i-col span="6">
            <FormItem label="欠费金额：" prop="ArrearsAmount">
              <Input v-model="formValidate.ArrearsAmount" placeholder="欠费金额" :readonly="true" />
            </FormItem>
          </i-col>
          <i-col span="6">
            <FormItem label="回款金额：" prop="RepaymentAmount">
              <Input v-model="formValidate.RepaymentAmount" placeholder="回款金额" :readonly="true" />
            </FormItem>
          </i-col>
          <i-col span="6">
            <FormItem label="剩余欠款：" prop="ResidueArrears">
              <Input v-model="formValidate.ResidueArrears" placeholder="剩余欠款" :readonly="true" />
            </FormItem>
          </i-col>
        </row>
        ...
        <row type="flex" justify="space-between" v-if="QXZX">
          <i-col span="6">
            <FormItem label="退费金额：" prop="RefundAmount">
              <Input v-model="formValidate.RefundAmount" placeholder="退费金额" :readonly="true" />
            </FormItem>
          </i-col>
        </row>
      ...
      <Input v-model="formValidate.Remark" :placeholder="QXZX==true?'终止执行原因':'备注'" type="textarea" :rows="4" style="margin-top:20px;" />
      <!-- ** -->
...
export default {
            ...
            return h("div", [
              h(
                "Select",
                {
                  ...
                  on: {
                    "on-change": event => {
                      that.hlsName = event.label;
                      that.chargeTableData[params.index].BeauticianID =
                        event.value;
                    }
                  }
                },
  ...
  methods: {
    ...
    getRowID(row) {
      ...
      that.RemainingTimes = row.RemainingTimes;
      that.ExecuteStatus = row.ExecuteStatus;
      that.ItemEndModels = row.ItemEndModels;
    ...
    checkCustomerOpenID(CustomerOpenID, time, xmName, storeName, hlsName) {
      let that = this;
      var data = {
        touser: CustomerOpenID,
        template_id: echartsCommon.CustomerOpenIDxmzx(),
        //服务通知中进入小程序的入口
