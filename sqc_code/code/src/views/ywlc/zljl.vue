      ...
      <Form :model="formItem" :label-width="0" inline @submit.native.prevent ref="searchForm" class="search-form">
        <row :gutter="20">
          ...
          <i-col span="3">
            ...
            <formItem prop="BeauticianID">
              <Select v-model="formItem.BeauticianID" :filterable="true" placeholder="护理师" @on-change="chooseYymd">
                <Option :value="item.ID" v-for="(item, index) in mrsList" :key="item.ID">{{item.Name}}</Option>
              </Select>
            </formItem>
  ...
  methods: {
    ...
    getList() {
      ...
      let data = {
        ...
        BeauticianID: that.formItem.BeauticianID,
      ...
      api.executionRecordGetListNew(data).then(response => {
        if (response.error_code === "Success") {
          let res = response.data;
