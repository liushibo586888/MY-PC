      ...
      <Form :model="treatmentReservation" :label-width="120" inline @submit.native.prevent width="600" ref="shopReservationType">
        ...
        <Row>
          <i-col span="24">
            ...
            <FormItem label="护理师：" prop="beautician" style="width:100%">
              <Select v-model="treatmentReservation.beautician" placeholder="请选择" @on-change="chooseBeautician">
                <Option :value="item.ID" v-for="(item, index) in beauticianList" :key="index">{{item.Name}}</Option>
              </Select>
            </FormItem>
          </i-col>
        </Row>
...
export default {
  ...
  methods: {
    chooseBeautician(value) {
      let that = this;
      that.treatmentReservation.beautician = value;
    },
    searchCustomerPhone() {
      let that = this;
  ...
  mounted() {
    ...
    that.$nextTick(() => {
      ...
      that.getBeauticiansByEmployeeID();
      // that.getStoreList()
    });
  },
  activated() {
