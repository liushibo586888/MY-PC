<template>
  <div class="yygn-container container">
        ...
        <row :gutter="20">
          ...
          <i-col span="12">
            <formItem class="btn-box">
              ...
              <Button type="primary" class="btn btn-purple" @click="cancelProject" v-if="showCancel"
                :disabled="!rowID">取消</Button>

              <Dropdown transfer @on-click="addPoups" v-if="shortcutMenuIndex > 0">
                <Button type="primary" class="btn btn-add" :disabled="!rowPhone||isDisable">
                  快捷菜单
                </Button>
                <DropdownMenu slot="list" v-show="isShowDD">
    ...
    <div class="table-box" id="tableBox">
      <Table :columns="columns1" :data="list" :height="setTableHeight" ref="mainTable" highlight-row stripe
        @on-current-change="getRowID"></Table>
    ...
    <!-- 选择预约类型弹窗 -->
    <Modal :mask-closable="false" v-model="reservationType" class="dialog" class-name="vertical-center-modal"
      v-if="showAdd">
      <div slot='header' class="header">
        选择预约类型
      </div>
      <Form :model="formValidate" :label-width="120" inline width="600" ref="formValidate1"
        @submit.native.prevent="chooseType">
        <Row>
          <i-col span="24">
            <FormItem prop="CustomerPhone" label="查询电话：" style="width:100%;" :rules="formValidateRule.CustomerPhone">
              ...
              <Input v-model="formValidate.CustomerPhone" placeholder="请输入" @on-blur="searchCustomerPhone()" autofocus
                ref="searchTel"></Input>
            </FormItem>
          </i-col>
        </Row>
        ...
        <Row>
          <i-col span="24">
            ...
            <FormItem label="预约门店：" prop="ReserveStoreID" style="width:100%" :rules="formValidateRule.ReserveStoreID"
              v-else>
      ...
      <div slot="footer" class="footer" style="padding-right:0;">
        <Button type="primary" class="btn" @click="chooseType" :loading="upLoading"
          :disabled="confirmReserveType">确定</Button>
      </div>
    </Modal>
    <!-- 到店预约 -->
    <Modal :mask-closable="false" v-model="shopReservationType" class="dialog" class-name="vertical-center-modal"
      v-if="showAdd">
      <div slot='header' class="header">
        到店预约
      </div>
      <Form :model="shopReservation" :label-width="120" inline @submit.native.prevent="addShopReservation" width="600"
        ref="shopReservationType">
        <Row>
          <i-col span="24">
            <FormItem prop="Date" label="预约日期：" style="width:100%;">
              <DatePicker type="datetime" :clearable="false" v-model="shopReservation.Date" style="width:100%;">
              </DatePicker>
            </FormItem>
          </i-col>
        </Row>
    ...
    <!-- 治疗预约 -->
    <Modal :mask-closable="false" v-model="treatmentReservationType" class="dialog treatmentReservation-dialog"
      class-name="vertical-center-modal" v-if="showAdd">
      <div slot='header' class="header">
        治疗预约
      </div>
      <Form :model="treatmentReservation" :label-width="120" inline @submit.native.prevent width="600"
        ref="shopReservationType">
        <Row>
          <i-col span="24">
            <FormItem prop="Date" label="预约日期：" style="width:100%;">
              <DatePicker @on-change="changeDate" type="date" :clearable="false" v-model="treatmentReservation.Date"
    ...
    <!-- 选择项目 -->
    <Modal :mask-closable="false" v-model="projectListModal" class="dialog choose-project"
      class-name="vertical-center-modal" width="500" v-if="showAdd">
      <div slot='header' class="header">
        选择项目
      </div>
      <div class="tab-box">
      ...
      <div class="search-list">
        <Input v-model="projectSearch" placeholder="请输入项目名称" style="width: 300px; margin: 10px 0 10px 4%;"
          icon="search"></Input>
      </div>
      <div class="list-content">
        <Spin size="large" fix v-if="projectShow"></Spin>
        <!-- 单条 已购-->
        <div class="item" v-for="(item, index) in customerPurchasedList" :key="index"
          v-if="projectTabIndex == 1 && customerPurchasedList.length > 0 && (item.ItemName.indexOf(projectSearch) > -1 || item.SimpleCode.indexOf(projectSearch) > -1)"
        ...
        <!-- 单条 未购-->
        <div class="item" v-for="(item, index) in reserveItemInfoList" :key="index"
          v-if="projectTabIndex == 2 && reserveItemInfoList.length>0 && (item.Name.indexOf(projectSearch) > -1 || item.SimpleCode.indexOf(projectSearch) > -1)"
          @click="chooseItem(item)">
          <div class="title">
        ...
        <!-- 未购买提示 -->
        <div v-if="customerPurchasedList.length > 0 || reserveItemInfoList.length > 0"
          style="height:50px; line-height:50px;text-align:center;color:#666;">
          暂无项目
        </div>
        <!-- ** -->
      </div>
    ...
    <!-- 护理师-->
    <Modal :mask-closable="false" v-model="beauticianModal" class="dialog choose-beautician"
      class-name="vertical-center-modal" width="650" style="padding-top:45px;" v-if="showAdd">
      <div slot='header' class="header">
        选择护理师
      </div>
      <!-- 选择日期 -->
      <div class="date-box" style="width:100%;padding-bottom: 16px;">
        <Row type="flex" justify="center">
          ...
          <i-col span="5" style="display:flex;align-items:center;justify-content:center;">
            <DatePicker :open="beauticianTime" :value="beauticianMes.date" type="date" confirm :clearable="false"
              @on-change="handleChange" @on-clear="handleClear" @on-ok="handleOk" v-if="showDateTime">
              <div class="date-wrapper"
                style="width:80px;text-align:center; background:#eee; height:27px;line-height:27px; border-radius:4px;cursor: pointer;"
                @click="handleClick">{{beauticianMes.date | formatDate}}</div>
            </DatePicker>
          </i-col>
          <i-col span="3">
      ...
      <div class="wrapper"
        style="height:334px; width:96%; margin: 0 auto; border-radius:4px; border:1px solid #ebebeb;">
        <Row style="width:100%; height:100%;">
          <i-col span="8" style="height:100%;"> -->
            <!-- 选择护理师列表 -->
            <div class="beautician-list"
              style="width:100%; height:100%; border-right: 1px solid #ebebeb; overflow-y:auto;">
              <div class="beautician-item" v-for="(i, index) in beauticianList" :key="index"
                @click="chooseBeautician(index)" :class="{'active' : beauticianIndex == index}">
                <div class="beautician-avatar" v-if="i.Image"
          ...
          <i-col span="16" style="height:100%;"> -->
            ...
            <div class="beautician-timer">
              ...
              <div class="content">
                <div class="item" v-for="(i,index) in appointmentList" :key="index"
                  :class="{'active': timeQuantumArr.indexOf(i.ID) > -1, 'disable': i.IsCanReserve == false}"
                  @click="chooseTimeSlot(i.ID, i.IsCanReserve)">
                  <Icon type="ios-circle-outline" v-if="timeQuantumArr.indexOf(i.ID) < 0" style="margin-right:10px;">
                  </Icon>
  ...
  export default {
        ...
        columns1: [
          {
            ...
            render: (h, params) => {
              ...
              return h(
                "div",
                {},
                moment(params.row.ReserveDate).format("YYYY-MM-DD HH:mm")
              );
            }
          },
          {
    ...
    methods: {
      ...
      getRowID(row) {
        ...
        if (row.Status == 3 || row.Status == 2) {//全部隐藏快捷开单
          that.isShowDD = false;
          that.isDisable = true;
        } else {
          that.isShowDD = true;
        ...
        if (row.ClientType != null || row.Status == 3) {//隐藏开单
          that.isShow = false;
        } else {
          that.isShow = true;
        }
