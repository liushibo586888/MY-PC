    ...
    <Modal :mask-closable="false" v-model="modal1" :styles="{top: '0px'}" class="dialog" width="1200" v-if="showAdd || showEdit">
      ...
      <Form ref="formValidate1" :model="formValidate" :label-width="120" @submit.native.prevent class="form">
        ...
        <row type="flex">
          <i-col span="5">
            <formItem label="退费日期：" prop="tfDate">
              <DatePicker class="DatePicker_time" type="date" :options="optionsDate" placeholder="退费日期" v-model="formValidate.tfDate" :clearable="false" :editable="false"></DatePicker>
            </formItem>
          </i-col>
          <i-col span="5">
  ...
  methods: {
    ...
    setData(res) {
      ...
      that.getStoreSceneList(res.ID).then(() => {
        ...
        vueData.CustomerPhone =
          res.Phone.substr(0, 3) + "****" + res.Phone.substr(7, 11);
        ...
        if (that.modalTitle == "浏览") {
          vueData.tfDate = res.Date;
          vueData.ZJID = res.SceneEmployee;
        } else {
          vueData.tfDate = new Date();
        }
      });
    },
    searchForm() {
    ...
    addForm() {
      ...
      if (that.formValidate.ZJID == "") {
        that.$Message.error("请选择总监");
        that.btnjinyong = false;
        return false;
      }
      let isBreak = false;
      ...
      let data = {
        ...
        date: echartsCommon.changeTimeHms(that.formValidate.tfDate),

        HandleBy: that.userMes.EmployeeID,
        OrderHandle: that.detailMessageData[0].OrderHandle,
    ...
    showDetailMessage(row) {
      ...
      // that.formValidate.tfDate = row.ChangeDate; //退费时间
      // 显示项目表格详细信息
      let a = {
        id: row.ID,
        xmmc: row.Name,
    ...
    async getStoreSceneList(CustomerID) {
      ...
      await api.getStoreSceneCust(data).then(res => {
