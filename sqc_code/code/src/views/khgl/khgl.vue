<template>
  <div class="khgl-container container">
    ...
    <Modal title="收货提醒" v-model="showRemind" class-name="vertical-center-modal">
...
export default {
  ...
  mounted() {
    ...
    //提醒收货,每个月25号之后,针对那些可怜的前台。前台太笨了，不收货提醒她到吐为止
    var myDate = new Date();
    var day = myDate.getDate();
    // that.showContent='<a href="www.baidu.com">badifa</a>'
    if (day >= 25) {
      let data = {
        EmployeeID: that.userMes.EmployeeID,
        storeId: that.userMes.StoreID
      };
      api.GetRemindReceiveGood(data).then(response => {
        if (response.error_code === "Success") {
          let res = response.data;
          if (res.length > 0) {
            let html =
              '<p class="mind">你有<span style="padding: 2px 5px;color: red;">2</span>条物资申请单没确认收货。请到<span style="padding: 2px 5px;color: red;">仓库管理-- 申请单列表</span>中及时收货!</p>';
