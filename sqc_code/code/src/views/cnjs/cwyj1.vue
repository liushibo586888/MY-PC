        ...
        <row :gutter="20">
          ...
          <i-col span="3">
            <FormItem prop="storeID">
              <Select v-model="formItem.storeID" placeholder="门店" :filterable="true" :label-in-value='true' @on-change='clickStore'>
    ...
    <Modal :mask-closable="false" v-model="modal1" :styles="{top: '20px'}" width='1200' class="dialog" ok-text='关闭' cancel-text=''>
      ...
      <Tabs v-model="tabValue" @on-click='tabClick'>
        ...
        <TabPane label="手工数" name="0">
          ...
          <div class="table-box" id="tableBox">
            <Table ref="sgsTable" :columns="columnsTab3" :data="cwyjDetailData" :height="tableHeight"></Table>
          </div>
        </TabPane>
  ...
  methods: {
    ...
    exportdosc() {
      ...
      window.location.href =
        ...
        that.modelType +
        "$AreaName=" +
        that.formItem.storeName;
    },
    //选择区域
    clickStore(item) {
      this.formItem.storeName = item.label;
    },
    //手工数明细 勾选事件
    checkboxchange(data) {
      this.cwyjDetailData = [];
